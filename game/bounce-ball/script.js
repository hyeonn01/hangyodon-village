...
const ball = { x: 100, y: 0, radius: 50, vy: 0, jumpPower: -12, jumpCount: 0, angle: 0 };
...
function createPlatform(x, isFirst = false) {
  let y = isFirst ? window.innerHeight - 200 : window.innerHeight - 150 - Math.random() * 250;
  const lastPlatform = platforms[platforms.length - 1];
  const wasLastTrap = lastPlatform ? lastPlatform.isTrap : false;
  const isTrap = !isFirst && !wasLastTrap && score >= 30 && Math.random() < 0.2;

  return {
    x: x, y: y, initialY: y,
    isTrap: isTrap,
    isBroken: false, 
    hasItem: !isFirst && !isTrap && Math.random() < 0.4,
    itemCollected: false,
    itemOffsetX: (Math.random() - 0.5) * 80,
    itemOffsetY: -50 - Math.random() * 120,
    isMoving: !isFirst && score >= 50 && Math.random() < 0.4,
...
function createParticles(x, y, color = "white", count = 5) {
  for (let i = 0; i < count; i++) {
    particles.push({
      x: x, y: y,
      vx: (Math.random() - 0.5) * (count > 10 ? 10 : 4), 
      vy: (Math.random() - 1) * (count > 10 ? 10 : 2),
      life: 1.0,
...
function drawPlatforms() {
  platforms.forEach(p => {
    ...
    ctx.save();
    if (p.isTrap) {
      // 필터를 사용하여 이미지의 투명도(Alpha) 영역은 건드리지 않고
      // 밝기를 낮추고 파란색 색조를 입힙니다. (가장 확실한 방법)
...
function drawBackground(timeFactor) {
  let bgSpeed = (0.5 + (score * 0.002)) * timeFactor;
...
function update(timestamp) {
  ...
  let gameSpeed = (3 + (score * 0.004)) * timeFactor;
  
  platforms.forEach(p => {
    p.x -= gameSpeed;
    if (p.isMoving) {
      p.moveOffset += 0.03 * timeFactor;
      p.y = p.initialY + Math.sin(p.moveOffset) * 50;
    }
  });
  ...
  platforms.forEach(p => {
    ...
    if (ball.x + 20 > p.x && ball.x - 20 < p.x + platformWidth && footPos > p.y && footPos < p.y + 30 && ball.vy >= 0) {
      if (p.isTrap) {
